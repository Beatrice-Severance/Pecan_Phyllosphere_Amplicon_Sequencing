...
Pipelines encompassing ITS and 16S community profiling for the pecan (Carya illinoinensis) phyllosphere, starting from demultiplexed reads. The scripts in this pipeline were originally run on the Alabama supercomputer and thus might make reproducibility difficult for users who do not utilize this system. The Alabama supercomputer recently shifted from DMC to ASAX. Currently, users can utilize the ASAX scripts to perform data processing, and these will be linked throughout where available. For reference, the DMC scripts previously used are linked here. Scripts for this pipeline are grouped in a folder along with the sample files from 2021 and 2022 leaf sequences extracted from pecan trees at the E.V. Smith facility in Alabama. The Alabama HPC utilizes a slurm queue system where jobs are submitted and run. Raw demultiplexed fungal reads are available via the [Sequence Read Archive](https://www.ncbi.nlm.nih.gov/sra/) (SRA) under [BioProject PRJNA1117508](https://www.ncbi.nlm.nih.gov/search/all/?term=PRJNA1117508). Bacterial reads will become available when processed.

# Fungal ITS Profiling

## Creating a [Samples](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/ASAX_Scripts/samples.txt) File
...
## [Strip Primers](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/ASAX_Scripts/1_strippingPrimers.sh)
...
## [Run Statistics](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/ASAX_Scripts/2_stat_results.sh)
...
## [Filter](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/ASAX_Scripts/3_filtering_and_trimming.sh)
...
## [Cluster](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/ASAX_Scripts/4_clustering.sh)
...
### [NBC Taxonomy](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/ASAX_Scripts/5_DADA2_NBC.sh)
This script requires the R script [dada2_assigntax_NBC.R](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/ASAX_Scripts/dada2_assigntax_NBC.R) which will use the database provided to create taxonomy based on the NBC algorithm. This script requires a large amount of memory in order to run, so should be taken into account when attempting to replicate this pipeline. 1 core and 32gb of memory produced parallel efficiency of 99.37% and memory efficiency of 60.63%. The script creates an .rds file which will allow users to view the taxonomy in R or RStudio. The database that was used for this script is located [here](https://doi.plutof.ut.ee/doi/10.15156/BIO/2483914). The following release was used for analysis: 29.11.2022
Mock sequences in NBC format can be found [here](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/Mock_Sequences/mocksequencesNBC.txt).

### [SINTAX Taxonomy](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/ASAX_Scripts/5_taxonomy_SINTAX.sh)
...
Mock sequences in SINTAX format can be found [here](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/Mock_Sequences/mocksequencesSINTAX.txt).

##
Combined, these steps provide the following output files that can be utilized in a phyloseq object in R:
- [OTU table](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/phyloseq_input/otu.table.csv)
- ITS taxonomy files (from SINTAX and [NBC](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/phyloseq_input/NBC.csv)
- [otus.fasta](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/phyloseq_input/otus_R1.fasta) file

## R Analysis: Fungi
R Analysis for this project starts with the creation of a phyloseq object. In addition to the files above, a [metadata](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/phyloseq_input/21-22_Metadata.csv) file will be necessary to run analysis. The [R Markdown file](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Fungi_ITS/EV_21-22_Fungi.Rmd) will execute the following steps:
- Load dependencies
...
- Load in [files](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/tree/main/Fungi_ITS/phyloseq_input) to create a phyloseq object
...
# Note! The pipelines listed below only encompass data analysis from 2021 alone and not 2022 data. This data should be incorporated when possible.

## [Merging Reads](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/Scripts/merge_reads.sh)
...
## [Stripping Primers](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/Scripts/stripping_primers.sh)
...
## [Run Statistics](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/Scripts/stats.sh)
...
## [Filter](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/Scripts/filtering.sh)
...
## [Cluster](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/Scripts/clustering.sh)
...
## [Mapping](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/Scripts/mapping.sh)
This is a script that will create an OTU table that can be used for further downstream analysis. A [Python script](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/Scripts/replacefastaheaders_filename.py) is used to edit filenames so that headers are appropriate for this analysis. The mapping script utilizes the input from the merge reads step and aligns these reads back to the clustered OTUs or zOTUs.

## [Taxonomy](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/Scripts/taxonomy.sh)
...
- [otu_table_16s.csv](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/phyloseq_input/otu_table_16s.csv)
- [16s_taxonomy.csv](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/phyloseq_input/16s_taxonomy.csv)
- [metadata2021.csv](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/phyloseq_input/metadata2021.csv)
- [otus.fasta](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/phyloseq_input/otus.fasta)

The [R markdown file](https://github.com/Beatrice-Severance/Pecan_Phyllosphere_Amplicon_Sequencing/blob/main/Bacteria_16S/Phyloseq_Analysis.Rmd) will execute the following steps:
- Load Dependencies
- Utilize a colorblind palette
